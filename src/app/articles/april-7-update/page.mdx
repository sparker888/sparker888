...
- **Original, unstructured keyword lists** – base prompt with simple, comma-separated keywords (no content).
...
I tested each approach against **Midjourney v7** (and sometimes v6.1) using various parameters like `--ar`, `--stylize`, `--c`, and `--q`. My primary goals were **realism**, **visual consistency**, and **clarity of style** and an overall final pleasing image.
...
4. **Avoid over-labeling** (like “Theme: …” or “Mood: …”) which can sometimes dilute emphasis (one strategy I tested).
...
**In short**, it’s the same attributes as the short comma-separated list of keywords that I originally used, but rephrased into a coherent sentence.
